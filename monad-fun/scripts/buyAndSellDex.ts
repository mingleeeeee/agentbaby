...
async function main() {
  ...
  const agentCall = new ethers.Contract(
    agentTokenAddress,
    agentTokenAbi,
    signer
  );
  const poolAddress = await agentCall.balancerPool();
  console.log("pool address:", poolAddress);

  // to get pool id, pool address is the balancer pool address
  const basePoolCall = new ethers.Contract(poolAddress, IBASE_POOL, signer);
  const poolId = await basePoolCall.getPoolId();
  console.log("poolId:", poolId);

  //  using gaming dex SDK to execute the swap
}
