    ...
    Ownable2StepUpgradeable
{
    ...
    address public kuruCreatedMarketAddress;
    ...
    uint16 public projectBuyTaxBasisPoints; // 0
    uint16 public projectSellTaxBasisPoints; // 0
    uint16 public swapThresholdBasisPoints; // 1
    ...
    address public projectTaxRecipient; // the address receiving project tax
    uint128 public projectTaxPendingSwap;
    address public vault; // Project supply vault address (fpair address)
     ...
     */
    // 只有owner或factory可以操作 owner() 是 token admin
    ...
    function initialize(
        address[2] memory integrationAddresses_, // 0: token admin address, 1: virtual token address
        bytes memory baseParams_, // token name, symbol
        ...
        // virtual address
        pairToken = integrationAddresses_[1];

        // set up kuru router to create market
        kuruRouterAddress = kuruRouterAddress_;

        // set up base token symbol
        ...
        // inside pool pair裡面剩餘的MEME tokens = lpSupply
        uint256 lpSupply = supplyParams.lpSupply * (10 ** decimals());
        // total supply - lpSupply = vaultSupply
        ...
        // NOTE: MONAD create a KURU MARKET
        kuruCreatedMarketAddress = _createPool();

        // set agent factory address
    ...
    function _createPool() internal returns (address kuruMarketProxyAddress) {
        ...
        // test
        require(
            kuruMarketProxyAddress != address(0),
            "Invalid kuruMarketProxyAddress address"
        );

        // 將 kuruMarketProxyAddress 加入到 _liquidityPools
        _liquidityPools.add(kuruMarketProxyAddress);
        emit LiquidityPoolCreated(kuruMarketProxyAddress);

        return (kuruMarketProxyAddress);
    }
    ...
    function _addInitialLiquidity(address lpOwner) internal {
        ...
        (
            ...
            uint96 spread
        ) = IKuruVault(kuruCreatedMarketAddress).getVaultParams();

        // test
        require(kuruAmmVault != address(0), "Invalid kuruAmmVault address");
    ...
    function isLiquidityPool(address queryAddress_) public view returns (bool) {
        ...
        // NEW: check if address is kuruCreatedMarketAddress (in _liquidityPools)
        return (queryAddress_ == kuruCreatedMarketAddress ||
    ...
    function transfer(
        ...
        _transfer(
            ...
            to,
            amount,
            // 如果owner是liquidity pool 或 to是liquidity pool 就要apply tax
            // 代表跟pool交易
    ...
    function transferFrom(
        ...
        _transfer(
            ...
            to,
            amount,
            // 如果from或to是liquidity pool 就要apply tax
            // 代表跟pool交易
    ...
    function _transfer(
        ...
        // trnasfer token 時處理tax
        // Process taxes
        // amountMinusTax 為 扣完tax之後的meme token數量
    ...
    //     // 如果有swap交易徵稅
    //     if (_tokenHasTax) {
    //         // check the meme token balance of this contract
    //         uint256 contractBalance = balanceOf(address(this)); // 這會是目前錢包裡面的tax balance, tax 會收 memeToken
    //         uint256 swapBalance = contractBalance;

    //         uint256 swapThresholdInTokens = (_totalSupply *
    //             swapThresholdBasisPoints) / BP_DENOM;

    // REMOVE: this part for swap tax
    //         if (
    //             // 檢查是否可以在這裡執行swap
    ...
    function distributeTaxTokens() external {
        if (projectTaxPendingSwap > 0) {
            ...
            _transfer(
                address(this), // from
                projectTaxRecipient, // to
                projectDistribution, // amount
                false
            );
        }
    }
