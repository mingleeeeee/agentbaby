...
contract WeightedPool is BaseWeightedPool, WeightedPoolProtocolFees {
    ...
    constructor(
        ...
        ProtocolFeeCache(
            protocolFeeProvider,
            ProtocolFeeCache.DELEGATE_PROTOCOL_SWAP_FEES_SENTINEL
        )
        WeightedPoolProtocolFees(params.tokens.length, params.rateProviders)
    {
        ...
        InputHelpers.ensureInputLengthMatch(
            numTokens,
            params.normalizedWeights.length
        );
        ...
        for (uint8 i = 0; i < numTokens; i++) {
            ...
            _require(
                normalizedWeight >= WeightedMath._MIN_WEIGHT,
                Errors.MIN_WEIGHT
            );
        ...
        _token4 = numTokens > 4 ? params.tokens[4] : IERC20(address(0));
        _token5 = numTokens > 5 ? params.tokens[5] : IERC20(address(0));
        _token6 = numTokens > 6 ? params.tokens[6] : IERC20(address(0));
        _token7 = numTokens > 7 ? params.tokens[7] : IERC20(address(0));

        _scalingFactor0 = _computeScalingFactor(params.tokens[0]);
        _scalingFactor1 = _computeScalingFactor(params.tokens[1]);
        _scalingFactor2 = numTokens > 2
            ? _computeScalingFactor(params.tokens[2])
            : 0;
        _scalingFactor3 = numTokens > 3
            ? _computeScalingFactor(params.tokens[3])
            : 0;
        _scalingFactor4 = numTokens > 4
            ? _computeScalingFactor(params.tokens[4])
            : 0;
        _scalingFactor5 = numTokens > 5
            ? _computeScalingFactor(params.tokens[5])
            : 0;
        _scalingFactor6 = numTokens > 6
            ? _computeScalingFactor(params.tokens[6])
            : 0;
        _scalingFactor7 = numTokens > 7
            ? _computeScalingFactor(params.tokens[7])
            : 0;
    ...
    function _beforeJoinExit(
        ...
        (
            uint256 protocolFeesToBeMinted,
            uint256 athRateProduct
        ) = _getPreJoinExitProtocolFees(
                invariant,
                normalizedWeights,
        ...
        return (
            supplyBeforeFeeCollection.add(protocolFeesToBeMinted),
            invariant
        );
    }
     ...
     * See https://forum.balancer.fi/t/reentrancy-vulnerability-scope-expanded/4345 for reference.
     */
    function _beforeProtocolFeeCacheUpdate()
        internal
        override
