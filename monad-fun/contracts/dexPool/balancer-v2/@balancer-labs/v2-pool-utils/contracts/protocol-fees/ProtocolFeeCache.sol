...
abstract contract ProtocolFeeCache is RecoveryMode {
    ...
    uint256 public constant DELEGATE_PROTOCOL_SWAP_FEES_SENTINEL =
        type(uint256).max;
    ...
    constructor(
        ...
        if (delegatedProtocolSwapFees) {
            ...
            _require(
                protocolSwapFeePercentage <=
                    protocolFeeProvider.getFeeTypeMaximumPercentage(
            ...
            emit ProtocolFeePercentageCacheUpdated(
                ProtocolFeeType.SWAP,
                protocolSwapFeePercentage
            );
        }
        ...
        _fixedProtocolSwapFeePercentage = delegatedProtocolSwapFees
            ? 0
            : protocolSwapFeePercentage;
    }
        ...
        if (feeType == ProtocolFeeType.SWAP) {
            return
                getProtocolSwapFeeDelegation()
                    ? _cache.swapFee
                    : _fixedProtocolSwapFeePercentage;
    ...
    function _updateProtocolFeeCache(
        ...
        uint256 currentValue = protocolFeeProvider.getFeeTypePercentage(
            feeType
        );
